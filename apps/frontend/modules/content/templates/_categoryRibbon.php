...
<div class="categories">
  <a href="<?php echo url_for('feed_'.$moduleName.'_category', array('category' => 'all')) ?>" class="cat_all rnd_3 <?php echo ($selected_cats[0] == 0 && $actionName == 'feed') ? 'on' : '' ?>">
    <div class="arrow"></div>
    all categories
  </a>
  <ul class="categories_filter">
    ...
    <li class="top <?php echo ($actionName == 'feed' && in_array($category['id'], $selected_cats)) ? 'on' : '' ?>" data-value="<?php echo $category['id'] ?>">
      <a href="<?php echo url_for('feed_'.$moduleName.'_category', array('category' => $category['name_slug'])) ?>" class="name top rnd_3">
        <div class="arrow"></div>
      ...
      <ul class="child1 rnd_3 hide">
        ...
        <li class="<?php echo ($actionName == 'feed' && in_array($child1['id'], $selected_cats)) ? 'on' : '' ?>" data-value="<?php echo $child1['id'] ?>">
          <a href="<?php echo url_for('feed_'.$moduleName.'_category', array('category' => $child1['name_slug'])) ?>" class="name rnd_3"><?php echo $child1['name'] ?></a>
          ...
          <ul class="child2 rnd_3 hide">
            ...
            <li class="<?php echo ($actionName == 'feed' && in_array($child2['id'], $selected_cats)) ? 'on' : '' ?>" data-value="<?php echo $child2['id'] ?>">
              <a href="<?php echo url_for('feed_'.$moduleName.'_category', array('category' => $child2['name_slug'])) ?>" class="name rnd_3"><?php echo $child2['name'] ?></a>
            </li>
            <?php endforeach ?>
          </ul>
          <?php endif ?>
